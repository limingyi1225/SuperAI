...
test('sanitizeModelIds filters out unknown model ids', () => {
    const result = sanitizeModelIds(['gemini-3-pro', 'not-a-real-model', 'gpt-5.2']);
...
test('sanitizeModelIds deduplicates model ids', () => {
    const result = sanitizeModelIds(['gemini-3-pro', 'gemini-3-pro']);
    assert.equal(result.length, 1);
    assert.equal(result[0], 'gemini-3-pro');
});

test('sanitizeModelIds returns empty array for non-array input', () => {
    assert.deepEqual(sanitizeModelIds(null), []);
    assert.deepEqual(sanitizeModelIds('gemini-3-pro'), []);
...
test('sanitizeModelIds filters non-string entries', () => {
    const result = sanitizeModelIds(['gemini-3-pro', 123, null, undefined, 'gpt-5.2']);
    assert.deepEqual(result.sort(), ['gemini-3-pro', 'gpt-5.2'].sort());
});
...
test('resolveInitialModels returns sanitized models for valid JSON with known ids', () => {
    const json = JSON.stringify(['gemini-3-pro', 'gpt-5.2']);
    const result = resolveInitialModels(json);
    assert.deepEqual(result, ['gemini-3-pro', 'gpt-5.2']);
});

test('resolveInitialModels filters invalid ids from mixed JSON input', () => {
    const json = JSON.stringify(['gemini-3-pro', 'not-real', 'gpt-5.2']);
    const result = resolveInitialModels(json);
    assert.ok(result.includes('gemini-3-pro'));
    assert.ok(result.includes('gpt-5.2'));
